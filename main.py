    ...
    for epoch in range(args.epochs):
        ...
        for input_ids, labels in train_dataloader:
            ...
            if args.attack_mode in ('pgd', 'fgsm'): # 使用对抗样本更新一轮参数
                ...
                delta.grad.zero_()
                step += 1
            # pred = np.squeeze(np.argmax(logits.cpu().detach().numpy(), axis=-1))
            # print(pred)
            ...
            if step % 1000 == 0:
                ...
                print('step: {}, acc: {:.4f}'.format(step, acc))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack_iters', type=int, default=5, help='pgd')
    parser.add_argument('--lr', type=float, default=1e-3, help ='learning rate')
    parser.add_argument('--alpha', type=float, default=0.04, help ='')
    parser.add_argument('--epsilon', type=float, default=0.1, help ='')
    parser.add_argument('--attack_mode', type=str, default='fgsm', help ='')
    ...
    # train_dataset = THUCNews(file_paths[0])
